...
describe('Auth Controller E2E', () => {
  ...
  const getToken = async (email: string, password: string, refreshToken: boolean = false) => {
    return (
  ...
  beforeAll(async () => {
    ...
    user = await createUser(firstName, 'user@example.com', hashedPassword);
  });
  ...
  describe('Public Routes', () => {
    ...
    it('Should Not Return New Tokens (No HashedRefreshToken in DB)', async () => {
      const email = 'joao@example.com';
      const user = await createUser(firstName, email, hashedPassword);
      const userToken = await getToken(email, password, true);
